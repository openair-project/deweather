#' @name shared_deweather_params
#' @title Shared deweather parameters
#' @description This is a central place for describing typical parameters. It
#'   ensures consistency throughout deweather.
#'
...
#'
#'   This function supports parallel processing using the `{mirai}` package. You
#'   will likely find that the performance of this function increases if
#'   "daemons" are set. The greatest benefits will be seen if you spawn as many
#'   daemons as you have cores on your machine, although one fewer than the
#'   available cores is often a good rule of thumb.
#'
#'   ```
...
#'   be easily saved to a `.png`, `.svg`, or other 'static' file format. To save
#'   a `ggplot2` plot, it is recommended to use the [ggplot2::ggsave()]
#'   function.
#'
#'   - `"plotly"`, for dynamic plotting. This engine produces HTML plots which
#'   are suitable for embedding into `quarto` or `rmarkdown` documents, or for
#'   use in `shiny` applications. These can be saved using functions like
