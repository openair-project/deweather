...
A note on model selection: by default, `tune_dw_model()` uses `selection_method = "pct_loss"` rather than simply picking the configuration with the lowest RMSE. This prefers simpler models (shallower trees, stronger regularisation) whose RMSE is within `pct_loss_limit = 2` percent of the minimum. The motivation is that for hourly air quality data, the marginal RMSE gain of a more complex model is typically smaller than measurement uncertainty, and simpler models are faster to use in [simulate_dw_met()]. Use `selection_method = "best"` to revert to strict minimum-RMSE selection.
